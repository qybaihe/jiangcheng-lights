...
export type FilmProps = {title:string; kind:'proof'|'final'; duration:number; shots:Shot[]; captions:Cue[]; voice:Voice[]; sounds?:Voice[]; gameAudio:boolean; music?:string; effects?:string; logo?:string; annotations?:(Cue&{detail?:string})[]};
...
const Subtitle:React.FC<{cue:Cue;onGame:boolean;onEvidence:boolean}>=({cue,onGame,onEvidence})=>{
 const f=useCurrentFrame(),{fps}=useVideoConfig();
 return <div className={`spoken-subtitle ${onGame?'on-game':''} ${onEvidence?'on-evidence':''}`} style={{opacity:envelope(f,cue.duration*fps,.06,.1,fps)}}>{cue.who&&<small>{cue.who}</small>}<span>{cue.text}</span></div>;
};
...
export const DemoFilm:React.FC<FilmProps>=(props)=>{
 ...
 return <AbsoluteFill className={`film-root ${final?'final-film':''}`}>
   ...
   {props.voice.map((v,i)=><Sequence key={`voice-${i}`} from={Math.round(v.from*fps)} durationInFrames={Math.ceil(v.duration*fps)} name="旁白"><Audio src={staticFile(v.src)} volume={v.volume??.95}/></Sequence>)}
   {props.sounds?.map((v,i)=><Sequence key={`sfx-${i}`} from={Math.round(v.from*fps)} durationInFrames={Math.round(v.duration*fps)} name="设计音效"><Audio src={staticFile(v.src)} volume={f=>envelope(f,v.duration*fps,.03,.15,fps)*(v.volume??.4)}/></Sequence>)}
   {props.music&&<Audio src={staticFile(props.music)} volume={f=>{
    const t=f/fps,fade=envelope(f,props.duration*fps,.8,1.35,fps);
    if(final){const source=props.shots.find(s=>s.type==='game'&&t>=s.from&&t<s.from+s.duration);return fade*.85*duckAt(t,props.voice,.56)*(source&&(source.gameVolume??0)>.7?.35:1);}
   ...
   {props.effects&&props.shots.filter(s=>s.type==='cg').map(s=><Sequence key={`fx-${s.id}`} from={Math.round(s.from*fps)} durationInFrames={Math.round(s.duration*fps)} name="江风环境"><Audio src={staticFile(props.effects!)} trimBefore={Math.round((s.sourceIn??0)*fps)} volume={f=>envelope(f,s.duration*fps,.2,.25,fps)*.20}/></Sequence>)}
 </AbsoluteFill>;
};
