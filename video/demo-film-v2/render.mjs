...
const out=path.join(root,'output/demo-film-v2'),mode=process.argv[2]??'proof';
...
const stillTimes=process.argv.includes('--skip-stills')?[]:(final?[3,12,17,20,24,28,30,33,37,45,53,64,70,79,85,88,91,98,105,115]:[3,7,11,16,25]);
...
const target=path.join(out,final?'江城有灯-比赛Demo-1080p-v2-render.mp4':'江城有灯-30秒风格样片-1080p-v2.mp4');
let last=-1;
await renderMedia({...common,codec:'h264',crf:18,x264Preset:'medium',imageFormat:'png',colorSpace:'bt709',pixelFormat:'yuv420p',audioCodec:'aac',audioBitrate:'192k',sampleRate:48000,encodingMaxRate:'16M',encodingBufferSize:'32M',concurrency:8,enforceAudioTrack:true,outputLocation:target,onProgress:({progress})=>{const n=Math.floor(progress*10);if(n!==last){last=n;console.log(`Render ${n*10}%`);}},metadata:{title:final?'江城有灯 · 比赛 Demo':'江城有灯 · 30秒视听风格样片（非最终参赛片）'}});
