...
const run=process.env.CAPTURE_RUN||'granny-12s-'+new Date().toISOString().replace(/[:.]/g,'-');
assert.match(run,/^[a-zA-Z0-9_-]+$/);
assert.ok(['localhost','127.0.0.1'].includes(new URL(base).hostname));
const out=fileURLToPath(new URL('../output/demo-film-v1/capture/'+run+'/',import.meta.url));
...
try{
 ...
 assert.equal(report.postSizeFix.capture.width,1920);assert.equal(report.postSizeFix.capture.height,1080);assert.equal(report.postSizeFix.capture.frameRate,30);
 ...
 report.selectedDestination=await evaluate(`document.querySelector('#map-destination').value`);assert.equal(report.selectedDestination,'granny');
 ...
 assert.equal(report.beforeStop.tracks.find(t=>t.kind==='video').settings.width,1920);assert.equal(report.beforeStop.tracks.find(t=>t.kind==='video').settings.height,1080);
 ...
 const b64=await evaluate(`new Promise(resolve=>{const r=new FileReader();r.onload=()=>resolve(r.result.slice(r.result.indexOf(';base64,')+8));r.readAsDataURL(__DEMO_CAPTURE.blob)})`);const recordedBytes=Buffer.from(b64,'base64');assert.equal(recordedBytes.length,report.recording.bytes);await writeFile(out+'walking-route-native-tab.webm',recordedBytes);await shot('02-after-route');await evaluate('__DEMO_CAPTURE.stream.getTracks().forEach(t=>t.stop())');
