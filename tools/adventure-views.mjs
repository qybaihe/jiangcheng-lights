...
const expected='/assets/index-Bs6mbhbl.js';
const dir=path.resolve('output/qa/adventure/release-Bs6mbhbl/views');
...
try{
 ...
 await face(0);await walk(1,26,'back-from-start-five-metres');await face(0);await tiltDelta(-105);await page.mouse.wheel(0,800);await sleep(250);await snap('south-gateway','From START, walked backward approximately 5m, north-facing view of the complete south gateway.');
 await walk(1,14,'through-south-gateway');await face(0);await tiltDelta(45);await snap('north-timber-gallery','After passing through the south gateway, looking north toward the timber gallery.');
